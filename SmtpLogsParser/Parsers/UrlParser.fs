...
let urlParser: Parser<string, unit> =
    ...
    let queryParamParser =
        sepBy
            (manySatisfy (fun c -> c <> '=' && c <> '&' && c <> ' ') .>> pchar '='
    ...
    let urlParser: Parser<string, unit> =
        pipe5 schemeParser domainParser portParser pathParser queryParser (fun scheme host port path query -> $"{scheme}{host}{port}{path}{query}")
        .>>. fragmentParser
        |>> fun (first, fragment) -> $"{first}{fragment}"
